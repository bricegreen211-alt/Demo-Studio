...
| **Two copies of the project on one machine** | The likeliest cause of "I updated but nothing changed". Whichever folder ran `npm install` last owns the Desktop / Start Menu launcher, and the extension is loaded from whichever folder you picked in `chrome://extensions` — they can be different copies at different commits. Find them all with `find ~ -maxdepth 6 -type d -name Demo-Studio -not -path "*/node_modules/*"`, keep the one outside OneDrive, delete the rest, then run `npm install` in the keeper and re-load the extension from its `extension/` folder. |
| **The app and the extension show different versions** | Whichever is older was never restarted. Settings names the one at fault: **Restart Demo Studio** means the app was left running through an update — quit it from the menu bar / tray icon and start it again. **Needs reloading** means the extension — reload it at `chrome://extensions`. Contributors: `npm run version:check` confirms the three manifests in the repo agree. |
